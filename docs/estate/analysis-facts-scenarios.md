...
- **S4. Scaffolding replay.** The estate's configuration exists as committed data (`bootstrap/terraform/`) and its seeding as a committed tool (`bootstrap/seed/seed.py`) fed by a local template packet — issue content is never committed — so a future estate can be stood up by rerunning both; the record (`bootstrap/estate.md`, `bootstrap/operations.md`) is the readable form of the same intent.
