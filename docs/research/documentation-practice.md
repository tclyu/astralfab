...
**Status:** provisional; governed by `AGENTS.md`. To be re-graded under the documentation standard v1 that is designed from it, never erased. Envelope: the `Documentation` issue.
...
1. **This estate's committed files** — `AGENTS.md`, `.staging/README.md`, `bootstrap/estate.md`, `bootstrap/operations.md`. Re-findable at those paths on the `integration` line.
...
- Committed content already falls into four persistence classes: timeless statements, closed dated records, live facts, and derived facts. The last two are forbidden in committed files. That is in `AGENTS.md` (Truth). Any documentation rule has to live with this. A document in the repository is either timeless or a closed record. Anything else belongs on a live surface or in staging.
- Hard wrapping was measured as pure cost here. Thirteen prose files were unwrapped. Roughly 250 line breaks came out. The word sequence was unchanged. The wrapping carried no information. That measurement is in the bootstrap record. The formatting rule now in `AGENTS.md` (Writing) closed the waste.
...
- Authorship cannot be inferred from the writing account here. It is recorded in the work itself, in signature blocks. That is in `AGENTS.md` (Identity). Documentation authorship rules have to build on that convention rather than invent a parallel one.
...
2. Require a small header a script can check: subject, status (provisional or standing), date, and governing convention. That extends the five requirements: the question, the sources, the split between checked facts and believed claims, the confidence, and the date. Scripts already proved they can enforce exact bytes. Dates stop silent decay. Checkable structure holds; taste does not.
